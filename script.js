...
function updateMainKPIs(dataBruta, selectedUnidades, startDate, endDate) {
    // --- CHAMADA DE DEPURAÇÃO ADICIONADA AQUI ---
    debugUnitNames(selectedUnidades);

    const normalizeText = (text) => text?.trim().toUpperCase().normalize("NFD").replace(/[\u00c0-\u024f]/g, "");
...
function debugUnitNames(selectedUnidades) {
    // Roda o teste apenas quando a unidade específica é selecionada
    if (selectedUnidades.length === 1 && selectedUnidades[0].includes('Volta Redonda')) {
        console.log("%c--- DEBUG DE NOMES DE UNIDADE ---", "color: yellow; font-size: 14px;");
        
        const selectedName = selectedUnidades[0];
        console.log("Filtro Selecionado:");
        logCharDetails('Filtro', selectedName);

        console.log("Verificando chaves na planilha de Metas que contêm 'Volta Redonda':");
        let foundMatch = false;
        metasData.forEach((value, key) => {
            const unitFromMeta = key.split('-')[0];
            if (unitFromMeta.toLowerCase().includes('volta redonda')) {
                console.log("Meta Key Encontrada:");
                logCharDetails('Meta', unitFromMeta);
                foundMatch = true;
            }
        });
        if (!foundMatch) {
            console.log("Nenhuma unidade contendo 'Volta Redonda' foi encontrada nas chaves da planilha de Metas.");
        }
        console.log("%c--- FIM DO DEBUG ---", "color: yellow; font-size: 14px;");
    }
}

function logCharDetails(prefix, str) {
    const codes = [];
    for (let i = 0; i < str.length; i++) {
        codes.push(str.charCodeAt(i));
    }
    console.log(`=> Texto: '${str}'`);
    console.log(`=> Comprimento: ${str.length}`);
    console.log(`=> Códigos dos Caracteres: [${codes.join(', ')}]`);
}
