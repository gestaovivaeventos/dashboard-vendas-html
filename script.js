...
// Lógica de inicialização foi movida para depois do login
document.addEventListener("DOMContentLoaded", () => {
    ...
    const attemptLogin = () => {
        ...
        if (unit) {
            userAccessLevel = unit; // Define o nível de acesso
            loginOverlay.style.display = "none"; // Esconde a tela de login
            dashboardWrapper.style.display = "flex"; // Mostra o dashboard
            initializeDashboard(); // Inicia o carregamento dos dados
        } else {
...
// Função que agora inicializa o dashboard DEPOIS do login
    ...
    try {
        ...
        if (allData && allData.length > 0) {
            ...
            [
                ...
                "kpi-section",
                "kpi-section-py",
                "chart-vvr-mes-section",
                "chart-cumulative-section",
                "table-section",
                "chart-monthly-vvr-section",
                "chart-yearly-stacked-section",
                "chart-monthly-stacked-section",
                "chart-yearly-ticket-section",
                "chart-monthly-ticket-section",
                "chart-yearly-contracts-section",
                "chart-monthly-contracts-section",
                "chart-monthly-adesoes-section",
                "chart-yearly-adesoes-stacked-section",
                "chart-monthly-adesoes-stacked-section",
                "consultor-table-section",
                "detalhada-adesoes-table-section",
                "fundos-detalhados-table-section",
            ].forEach((id) => {
            ...
            populateFilters(); // Esta função agora vai respeitar o userAccessLevel
            addEventListeners();
            updateDashboard();
        } else {
            loader.innerHTML =
                "Nenhum dado de vendas encontrado ou falha ao carregar. Verifique o console (F12) para mais detalhes.";
        }
    } catch (error) {
...
function populateFilters() {
    ...
    if (userAccessLevel === "ALL_UNITS") {
        // Acesso de franqueadora: mostra todas as unidades
        ...
        // Acesso de franqueado: mostra e seleciona apenas a sua unidade
        unidadeFilter.append($("<option>", { value: userAccessLevel, text: userAccessLevel, selected: true }));

        unidadeFilter.multiselect({
            buttonWidth: "100%",
            onDropdownShown: function(event) { // Truque para desabilitar
                $(event.target).find("button.multiselect").prop('disabled', true);
                $(event.target).find(".multiselect-container").removeClass('show');
            }
        });
        // Desabilita o dropdown para o franqueado
    ...
    document.getElementById("start-date").value = inicioMes
        .toISOString()
        .split("T")[0];
    document.getElementById("end-date").value = fimMes
        .toISOString()
        .split("T")[0];
}

// O RESTANTE DO SEU CÓDIGO JAVASCRIPT PERMANECE IGUAL
// (fetchAllSalesDataFromSheet, fetchMetasData, updateDashboard, etc...)
// Cole todo o restante do seu script.js original aqui, sem nenhuma outra alteração.
...
    console.error(
      "ID da Planilha de Vendas, Nome da Aba ou Chave de API não configurados.",
    );
    return [];
  }
...
    if (!response.ok) {
      console.error(
        "Erro ao buscar dados de vendas da planilha:",
        await response.json(),
      );
      return [];
    }
...
      console.error(
        "Colunas essenciais (nm_unidade, dt_cadastro_integrante, vl_plano) não foram encontradas.",
      );
      return [];
    }
...
          vl_plano:
            parseFloat(String(row[valorIndex] || "0").replace(",", ".")) || 0,
          venda_posvenda:
            tipoVendaIndex !== -1 ? row[tipoVendaIndex] || "VENDA" : "N/A",
          indicado_por:
            indicadoPorIndex !== -1 ? row[indicadoPorIndex] || "N/A" : "N/A",
          codigo_integrante:
            codigoIntegranteIndex !== -1
              ? row[codigoIntegranteIndex] || "N/A"
              : "N/A",
          nm_integrante:
            nomeIntegranteIndex !== -1
              ? row[nomeIntegranteIndex] || "N/A"
              : "N/A",
...
    console.error(
      "ID da Planilha, Nome da Aba FUNDOS ou Chave de API não configurados.",
    );
    return [];
  }
...
    if (!response.ok) {
      console.error(
        "Erro ao buscar dados de fundos da planilha:",
        await response.json(),
      );
      return [];
    }
...
      console.error(
        "Colunas essenciais (nm_unidade, id_fundo, dt_contrato) não foram encontradas na planilha FUNDOS.",
      );
      return [];
    }
...
          dt_cadastro:
            dtCadastroIndex !== -1 ? parsePtBrDate(row[dtCadastroIndex]) : null,
          tipo_servico:
            tipoServicoIndex !== -1 ? row[tipoServicoIndex] || "N/A" : "N/A",
          instituicao:
            instituicaoIndex !== -1 ? row[instituicaoIndex] || "N/A" : "N/A",
          dt_baile:
            dtBaileIndex !== -1 ? parsePtBrDate(row[dtBaileIndex]) : null,
        };
      })
      .filter(Boolean);
  } catch (error) {
...
    const month = String(d.dt_cadastro_integrante.getMonth() + 1).padStart(
      2,
      "0",
    );
...
    if (
      unitsToConsider.includes(unidade) &&
      metaDate >= startDate &&
      metaDate < endDate
    ) {
...
  const percentPosVendas =
    metaPosVendas > 0 ? realizadoPosVendas / metaPosVendas : 0;
  const totalColor = getColorForPercentage(percentTotal);
  document.getElementById("kpi-total-realizado").textContent =
...
  document.getElementById("kpi-total-progress").style.backgroundColor =
    totalColor;
  document.getElementById("kpi-total-progress").style.width =
    `${Math.min(percentTotal * 100, 100)}%`;
  const vendasColor = getColorForPercentage(percentVendas);
  document.getElementById("kpi-vendas-realizado").textContent =
    formatCurrency(realizadoVendas);
  document.getElementById("kpi-vendas-meta").textContent =
...
  document.getElementById("kpi-vendas-progress").style.backgroundColor =
    vendasColor;
  document.getElementById("kpi-vendas-progress").style.width =
    `${Math.min(percentVendas * 100, 100)}%`;
  const posVendasColor = getColorForPercentage(percentPosVendas);
  document.getElementById("kpi-posvendas-realizado").textContent =
    formatCurrency(realizadoPosVendas);
  document.getElementById("kpi-posvendas-meta").textContent =
    formatCurrency(metaPosVendas);
...
  document.getElementById("kpi-posvendas-progress").style.backgroundColor =
    posVendasColor;
  document.getElementById("kpi-posvendas-progress").style.width =
    `${Math.min(percentPosVendas * 100, 100)}%`;
}
function updatePreviousYearKPIs(
  dataBruta,
  selectedUnidades,
  startDate,
  endDate,
...
    metaPosVendas > 0 ? realizadoPosVendas / metaPosVendas : 0;
  const totalColor = getColorForPercentage(percentTotal);
  document.getElementById("kpi-total-realizado-py").textContent =
    formatCurrency(realizadoTotal);
  document.getElementById("kpi-total-meta-py").textContent =
    formatCurrency(metaTotal);
  const totalPercentEl = document.getElementById("kpi-total-percent-py");
  totalPercentEl.textContent = formatPercent(percentTotal);
  totalPercentEl.style.color = totalColor;
  document.getElementById("kpi-total-progress-py").style.backgroundColor =
    totalColor;
  document.getElementById("kpi-total-progress-py").style.width =
    `${Math.min(percentTotal * 100, 100)}%`;
  const vendasColor = getColorForPercentage(percentVendas);
  document.getElementById("kpi-vendas-realizado-py").textContent =
    formatCurrency(realizadoVendas);
  document.getElementById("kpi-vendas-meta-py").textContent =
    formatCurrency(metaVendas);
  const vendasPercentEl = document.getElementById("kpi-vendas-percent-py");
  vendasPercentEl.textContent = formatPercent(percentVendas);
  vendasPercentEl.style.color = vendasColor;
  document.getElementById("kpi-vendas-progress-py").style.backgroundColor =
    vendasColor;
  document.getElementById("kpi-vendas-progress-py").style.width =
    `${Math.min(percentVendas * 100, 100)}%`;
  const posVendasColor = getColorForPercentage(percentPosVendas);
  document.getElementById("kpi-posvendas-realizado-py").textContent =
    formatCurrency(realizadoPosVendas);
  document.getElementById("kpi-posvendas-meta-py").textContent =
    formatCurrency(metaPosVendas);
  const posVendasPercentEl = document.getElementById(
    "kpi-posvendas-percent-py",
  );
  posVendasPercentEl.textContent = formatPercent(percentPosVendas);
  posVendasPercentEl.style.color = posVendasColor;
  document.getElementById("kpi-posvendas-progress-py").style.backgroundColor =
    posVendasColor;
  document.getElementById("kpi-posvendas-progress-py").style.width =
    `${Math.min(percentPosVendas * 100, 100)}%`;
}
...
  const dataParaGrafico = allData.filter(
    (d) =>
      (selectedUnidades.length === 0 ||
        selectedUnidades.includes(d.nm_unidade)) &&
...
  const dataBrutaFiltrada = allData.filter(
    (d) =>
      (selectedUnidades.length === 0 ||
        selectedUnidades.includes(d.nm_unidade)) &&
...
  dataAgregadaComVendas.forEach((item) => {
    const key = `${item.unidade}-${item.periodo}`;
...
      metaDate >= startDate &&
      metaDate < endDate &&
      !combinedDataMap.has(mapKey)
    ) {
      combinedDataMap.set(mapKey, {
        unidade: unidade,
        periodo: periodo,
        realizado_vvr: 0,
        realizado_adesoes: 0,
...
  const dataBrutaFiltradaPY = allData.filter(
    (d) =>
      (selectedUnidades.length === 0 ||
        selectedUnidades.includes(d.nm_unidade)) &&
...
  const allYearPeriodos = Array.from(
    { length: 12 },
    (_, i) => `${anoVigente}-${String(i + 1).padStart(2, "0")}`,
  );
...
    realizadoValues = allYearPeriodos.map(
      (p) => chartDataMap.get(p).realizado_vendas,
    );
    metaValues = allYearPeriodos.map((p) => chartDataMap.get(p).meta_vendas);
  } else if (currentVvrChartType === "posvendas") {
    realizadoValues = allYearPeriodos.map(
      (p) => chartDataMap.get(p).realizado_posvendas,
    );
    metaValues = allYearPeriodos.map((p) => chartDataMap.get(p).meta_posvendas);
  } else {
    realizadoValues = allYearPeriodos.map(
      (p) =>
        chartDataMap.get(p).realizado_vendas +
        chartDataMap.get(p).realizado_posvendas,
    );
...
  vvrVsMetaPorMesChart = new Chart(
    document.getElementById("vvrVsMetaPorMesChart"),
    {
      type: "bar",
      data: {
...
    "cumulative-chart-selector",
  );
  const unitsToConsider =
    selectedUnidades.length > 0
      ? selectedUnidades
      : [...new Set(allData.map((d) => d.nm_unidade))];
  const filteredHistoricalData = historicalData.filter((d) =>
    unitsToConsider.includes(d.nm_unidade),
  );
  const salesByYearMonth = {};
  const uniqueYears = [
    ...new Set(
      filteredHistoricalData.map((d) => d.dt_cadastro_integrante.getFullYear()),
    ),
  ].sort();
...
    selectorContainer.querySelectorAll("button.active"),
...
  cumulativeVvrChart = new Chart(
    document.getElementById("cumulativeVvrChart"),
    {
      type: "line",
...
  yearlyStackedChart = new Chart(
    document.getElementById("yearlyStackedChart"),
    {
      type: "bar",
      data: {
        labels: years,
...
  document.getElementById("monthly-stacked-title").textContent =
    `Venda Realizada Total Mensal (${year})`;
  const salesByMonth = Array(12)
    .fill(0)
    .map(() => ({ vendas: 0, posVendas: 0 }));
  const normalizeText = (text) =>
    text
      ?.trim()
      .toUpperCase()
...
  monthlyStackedChart = new Chart(
    document.getElementById("monthlyStackedChart"),
    {
      type: "bar",
      data: {
        labels: monthLabels,
...
          callbacks: {
            label: (context) =>
              `Ticket Médio: ${formatCurrency(context.parsed.x)}`,
          },
        },
      },
      scales: {
        // Adicionamos a configuração de escalas
        x: {
          beginAtZero: true,
...
  document.getElementById("monthly-ticket-title").textContent =
    `Ticket Médio Mensal (${year})`;

  const ticketByMonth = Array(12)
    .fill(0)
    .map(() => ({ totalValor: 0, totalAdesoes: 0 }));
...
  const monthlyTicketData = ticketByMonth.map((m) =>
    m.totalAdesoes > 0 ? m.totalValor / m.totalAdesoes : 0,
  );
  const monthLabels = [
    "jan",
    "fev",
...
  monthlyTicketChart = new Chart(
    document.getElementById("monthlyTicketChart"),
    {
      type: "bar",
      data: {
        labels: monthLabels,
...
            callbacks: {
              label: (context) =>
                `Ticket Médio: ${formatCurrency(context.parsed.y)}`,
            },
          },
        },
        scales: {
...
  yearlyContractsChart = new Chart(
    document.getElementById("yearlyContractsChart"),
    {
      type: "bar",
      data: {
        labels: years,
...
            callbacks: {
              label: (context) =>
                `Contratos: ${context.parsed.x.toLocaleString("pt-BR")}`,
            },
          },
        },
        onClick: (event, elements) => {
...
  document.getElementById("monthly-contracts-title").textContent =
    `Contratos Realizados Total Mensal (${year})`;
...
  monthlyContractsChart = new Chart(
    document.getElementById("monthlyContractsChart"),
    {
      type: "bar",
      data: {
        labels: monthLabels,
...
            callbacks: {
              label: (context) =>
                `Contratos: ${context.parsed.y.toLocaleString("pt-BR")}`,
            },
          },
        },
        scales: {
...
      let realizado = 0;
      let meta = 0;
      const vendasDoPeriodo = allData.filter(
        (v) =>
          v.nm_unidade === d.unidade &&
          `${v.dt_cadastro_integrante.getFullYear()}-${String(v.dt_cadastro_integrante.getMonth() + 1).padStart(2, "0")}` ===
            d.periodo,
      );
      if (currentTableDataType === "vendas") {
        realizado = vendasDoPeriodo
          .filter((v) => normalizeText(v.venda_posvenda) === "VENDA")
          .reduce((sum, v) => sum + v.vl_plano, 0);
        meta = d.meta_vvr_vendas;
      } else if (currentTableDataType === "posvendas") {
        realizado = vendasDoPeriodo
          .filter((v) => normalizeText(v.venda_posvenda) === "POS VENDA")
          .reduce((sum, v) => sum + v.vl_plano, 0);
...
          title: `Relatorio_Vendas_${new Date().toLocaleDateString("pt-BR")}`,
...
                if (column === 4) {
                  return (
                    parseFloat(data.replace("%", "").replace(",", ".").trim()) /
                    100
                  );
                }
                return data;
...
      document
        .querySelectorAll(".page-navigation button")
...
  document
    .querySelectorAll("#chart-vvr-mes-section .chart-selector button")
    .forEach((button) => {
      button.addEventListener("click", () => {
        document
          .querySelectorAll("#chart-vvr-mes-section .chart-selector button")
...
    });
  document
    .querySelectorAll("#table-section .chart-selector button")
    .forEach((button) => {
      button.addEventListener("click", () => {
...
  monthlyAdesoesChart = new Chart(
    document.getElementById("monthlyAdesoesChart"),
    {
      type: "bar",
      data: {
        labels: monthLabels,
...
  const adesoesPosVendasAnual = years.map(
    (year) => adesoesByYear[year].posVendas,
  );

  if (yearlyAdesoesStackedChart) yearlyAdesoesStackedChart.destroy();
  yearlyAdesoesStackedChart = new Chart(
    document.getElementById("yearlyAdesoesStackedChart"),
    {
      type: "bar",
      data: {
        labels: years,
...
  document.getElementById("monthly-adesoes-stacked-title").textContent =
    `Adesões por Tipo (Mensal ${year})`;
  const adesoesByMonth = Array(12)
    .fill(0)
    .map(() => ({ vendas: 0, posVendas: 0 }));
  const normalizeText = (text) =>
    text
...
  monthlyAdesoesStackedChart = new Chart(
    document.getElementById("monthlyAdesoesStackedChart"),
    {
      type: "bar",
      data: {
        labels: monthLabels,
...
function updateFundosDetalhadosTable(
  fundosData,
  selectedUnidades,
  startDate,
  endDate,
